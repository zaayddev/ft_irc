...
auth_response = requests.post(token_url, headers={"Authorization": f"Basic {auth_header}"}, data={"grant_type": "client_credentials"})
...
search_response = requests.get(search_url, headers={"Authorization": f"Bearer {access_token}"}, params={"q": {artist_name}, "type": "artist", "limit": 1})
...
tracks_response = requests.get(tracks_url, headers={"Authorization": f"Bearer {access_token}"}, params={"country": "US"})
